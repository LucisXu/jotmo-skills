...
- **业务数据存储（优先选择）**
  - Default：**MongoDB**
  - 用途：大部分业务数据、快记数据、AI 会话、音频处理记录、用户配置等
  - 优势：灵活 Schema、易扩展、适合快速迭代

- **事务性存储（严格事务要求时使用）**
  - Default：**MySQL 8.0+**
  - Alternatives：PostgreSQL
  - **仅在以下场景使用**：
    - 需要跨表强一致性事务（如支付、订单状态流转）
    - 需要复杂 JOIN 查询
    - 有严格的 ACID 要求
  - 用途：订单、支付、会员权益等需要强事务保证的场景

> ⚠️ **存储选择原则**：优先 MongoDB，只有在确实需要严格事务性保证时才使用 MySQL。不要因为"习惯"而选择 MySQL。
